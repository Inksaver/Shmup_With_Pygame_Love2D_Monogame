...
import shared, mob, bullet, circle, shield, explosion, powerup, leaderboard, menu
...
def shoot() -> None:
	...
	if data.allow_new_bullet and not player.hidden:	# has enough time passed to fire a new bullet?
		if shared.audio_present:
			sounds["shoot"].stop()
...
def load() -> None:
	...
	data.lb = leaderboard.Leaderboard()				# create instance of Leaderboard class
...
def update() -> None:
	...
	if quit:
		...
		if shared.gamestate == shared.gamestates["play"]:
			...
			menu.update(keystate, key_down)							# so update the 'menu' class
			
		elif shared.gamestate == shared.gamestates["leaderboard"]:	# current gamestate = 'leaderboard'
			data.lb.update(keystate, key_down)						# so update the 'leaderboard' class
...
def draw() -> None:
	...
	if shared.gamestate == shared.gamestates["play"]:
		...
		data.lb.draw()														# so draw the leaderboard class
