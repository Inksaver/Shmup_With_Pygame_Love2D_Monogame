...
local Lb 				= require "leaderboard"	-- import Leaderboard class
...
local Leaderboard		= nil		-- used for Leaderboard object
...
function love.textinput(t)
    --[[ Love2D special function to get text input ]]
	if Shared.gamestate == Shared.gamestates["leaderboard"] then
		Shared.inputText = Shared.inputText .. t:upper()
	end
end
...
function love.keypressed(key)
	...
	if Shared.gamestate == Shared.gamestates["leaderboard"] then
		if key == "backspace" then
			...
			local byteoffset = utf8.offset(Shared.inputText, -1)

			if byteoffset then
				-- remove the last UTF-8 character.
				-- string.sub operates on bytes rather than UTF-8 characters, so we couldn't do string.sub(text, 1, -2).
				Shared.inputText = string.sub(Shared.inputText, 1, byteoffset - 1)
			end
		elseif key == "return" then
			Leaderboard:addEntry()
		end
	elseif Shared.gamestate == Shared.gamestates["menu"] then
		if key == "return" then
...
function love.update(dt)
	...
	if quit or Shared.gamestate == Shared.gamestates["quit"] then
		...
		elseif Shared.gamestate == Shared.gamestates["leaderboard"] then
