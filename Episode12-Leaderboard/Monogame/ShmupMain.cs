...
namespace Shmup
{
    public class ShmupMain : Game
    {
        ...
        {
            /// similar to Love2D love.textinput(t)
            /// This event handler is created in the Initialize() method
            Keys pressedKey = args.Key;
            char character = args.Character;
            if (Shared.GameState == Shared.GameStates["menu"])
            {
                ...
                {
                    Shared.PlayMusic = !Shared.PlayMusic;
                    if (Shared.PlayMusic)
            ...
            else if (Shared.GameState == Shared.GameStates["leaderboard"])
            {
                ...
                {
                    if (Shared.InputText.Length > 1)
                    {
                        Shared.InputText = Shared.InputText.Substring(0, Shared.InputText.Length - 1);
                    }
                    else if (Shared.InputText.Length == 1)
                        Shared.InputText = "";
                }
                else if (pressedKey == Keys.Enter)
                    leaderboard.AddEntry();
                else
                    Shared.InputText += character.ToString().ToUpper();
        ...
        {
            /// This function returns the keyboard directly
            /// and provides the boolean value quit via the 'out' in parameters
            quit = false;
            ...
            for (int i = Mobs.Count - 1; i >= 0; i--)           // outer loop checks mobs
            {
                ...
                for (int j = Bullets.Count - 1; j >= 0; j--)    // inner loop checks Bullets
                {
                    ...
                    if (destroy)
                    {
                        ...
                        Mobs[i].Reset();        // re-deploy mob 
                        Bullets.RemoveAt(j);    // remove bullet
                    }
                }
            }
        ...
        public void Shoot()
        {
            ...
            if (allowNewBullet && !Player.Hidden)
            {
                soundEffectInstances["shoot"].Stop();
                if (Player.Power == 1)
                    Bullets.Add(new Bullet(bulletImg, "centre"));
                else
                {
                    Bullets.Add(new Bullet(bulletImg, "left"));
                    Bullets.Add(new Bullet(bulletImg, "right"));
                }
        ...
        protected override void Initialize()
        {
            ...
            Window.TextInput += TextInputHandler; // add event handler for keyboard input
            base.Initialize();
        }
        protected override void LoadContent()
        {
            ...
            LoadFonts();
            LoadAnimations();
            Player.SetProperties(playerimg: playerImg, spd: 300f, scl: 0.5f);
            random = new Random();
            ...
            //MediaPlayer.Play(Shared.BackgroundMusic);
        ...
        protected override void Update(GameTime gameTime)
        {
            ...
            {
                leaderboard.Update(keyboardState);
            }
        ...
        protected override void Draw(GameTime gameTime)
        {
            ...
            else if (Shared.GameState == Shared.GameStates["play"])
			{
                ...
                DrawLives(Shared.WIDTH - 100, 5, Player.Lives, 0.3f);
            ...
            if (Shared.Debug)
            {
                Shared.DrawString(spriteBatch: spriteBatch, text:"Debug mode", size:"size14", posX: 10, posY: Shared.HEIGHT - 24, color:Color.Yellow, align:"left" );
            }
            spriteBatch.End();
            base.Draw(gameTime);
        }
