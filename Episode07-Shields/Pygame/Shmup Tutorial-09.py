...
import shared, player, mob, bullet, shield
	...
	for i in range(len(bullets) -1, -1, -1):
		...
		for j in range(len(mobs) -1, -1, -1):
			...
			try:
				...
				pass					
			if destroy:
				shared.score += 50 - mobs[j].get_circle().radius					# higher score for small meteor
...
def load() -> None:
	...
	shared.debug = False # moved to stop background music in debug mode
	load_images()
	load_audio()
