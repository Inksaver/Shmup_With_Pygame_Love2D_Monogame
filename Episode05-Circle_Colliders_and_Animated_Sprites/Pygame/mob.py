...
class Mob():
	...
	def reset(self) -> None:
		...
		self.speedy = random.randrange(10, 100) #50,400
		self.speedx = random.randrange(-20, 20)# -200, 200
		if shared.debug:
	...
	def rotate(self, dt):
		...
		if self.time_passed >= 0.05:
			...
			''' pygame messes up images when they are rotated
			    so rotate a copy of the original.
				Also use the centre of the original rectangle
				and set the new rectangle centre to the same value
			'''
