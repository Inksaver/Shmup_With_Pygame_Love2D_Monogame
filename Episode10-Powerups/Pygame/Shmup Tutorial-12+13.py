...
import shared, player, mob, bullet, circle, shield, explosion, powerup
	...
	for i in range(len(bullets) -1, -1, -1):
		...
		for j in range(len(mobs) -1, -1, -1):
			...
			if destroy:
				...
				if random.random() > 0.9 or shared.debug: # generates random decimal between 0 and 1
					powerups.append(powerup.Pow(powerup_images, mobs[j].get_rect().center))							
				mobs[j].reset()
				if len(bullets) == 0:
					break
...
def check_powerup_player_collisions(dt:float) -> None:
	...
	for i in range(len(powerups) - 1, -1, -1):
		#if collides(mob.get_rect(), shared.player.get_rect()):
		if circle_collides(powerups[i].get_circle(), player.circle):
			if shared.audio_present:
				sounds['shoot'].stop()					
...
def shoot() -> None:
	...
	if data.allow_new_bullet and not player.hidden:
		if shared.audio_present:
			sounds["shoot"].stop()
		if player.power == 1:	
			bullets.append(bullet.Bullet(data.bullet_img, player.rect.centerx, player.rect.top))
		elif player.power >= 2:
			bullets.append(bullet.Bullet(data.bullet_img, player.rect.left, player.rect.centery))
			bullets.append(bullet.Bullet(data.bullet_img, player.rect.right, player.rect.centery))	
