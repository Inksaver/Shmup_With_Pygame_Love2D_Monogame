--[[ 
Lua / Love2D equivalent of Python / Pygame Shmup Episode 12+13
...
local function checkPowerupPlayerCollisions(dt)
	...
	for i = #powerups, 1, -1 do
		--if Player.rect:intersects(powerups[i]:getRect()) then
		if Player.circle:intersects(powerups[i]:getCircle()) then
			love.audio.stop(sounds.shoot)					
			if powerups[i].type == 'shield' then
...
local function shoot()
	...
	if allowNewBullet and not Player.hidden then								 -- has enough time passed to fire a new bullet?
		...
		if Player.power == 1 then
			...
			table.insert(bulletList, Bullet(Player, sprites.bullet, 'left'))  -- add a new bullet passing Player, width, height
			table.insert(bulletList, Bullet(Player, sprites.bullet, 'right')) -- add a new bullet passing Player, width, height
		end
...
function love.update(dt)
	...
	if not sounds.background:isPlaying() and not Shared.debug then
		love.audio.play(sounds.background)
	end
